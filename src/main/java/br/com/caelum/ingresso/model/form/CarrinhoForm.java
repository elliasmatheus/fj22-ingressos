...
public class CarrinhoForm {
	...
	public List<Ingresso> toIngressos(SessaoDao sessaoDao, LugarDao lugarDao){
		return this.ingressos.stream().map(ingresso -> toIngresso(sessaoDao, lugarDao, ingresso)).collect(Collectors.toList());
	}

	private Ingresso toIngresso(SessaoDao sessaoDao, LugarDao lugarDao, Ingresso ingresso) {
